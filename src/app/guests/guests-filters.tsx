// src/app/guests/guests-filters.tsx
"use client";
...
export default function GuestsFilters() {
  ...
  useEffect(() => {
    ...
    window.addEventListener("guests:pages", handler as any);
    return () => window.removeEventListener("guests:pages", handler as any);
  }, []);
  ...
  return (
    <div className="rounded-xl border border-white/10 bg-slate-900/50 p-4 flex flex-wrap gap-3">
      ...
      <div className="ml-auto flex items-center gap-2">
        ...
        <span className="text-white/70 text-sm">Page {page}{pages ? ` / ${pages}` : ""}</span>
        <button
          onClick={() => go(page + 1)}
          className="h-9 px-3 rounded-md border border-white/15 text-white/80 disabled:opacity-40"
